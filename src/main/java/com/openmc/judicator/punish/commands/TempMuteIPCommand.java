...
public class TempMuteIPCommand {
    ...
    public TempMuteIPCommand(Judicator judicator) {
    ...
    public void register() {
        ...
        final CommandMeta commandMeta = commandManager.metaBuilder("tempmuteip")
                .plugin(judicator)
                .build();

        final LiteralCommandNode<CommandSource> node = BrigadierCommand.literalArgumentBuilder("tempmuteip")
                .requires(source -> source.hasPermission(PunishPermissions.TEMPMUTEIP.getPermission()) || source.hasPermission(PunishPermissions.ADMIN.getPermission()))
    ...
    private int wrongUsage(CommandContext<CommandSource> context) {
        ...
        final TextComponent text = PunishUtils.getMessage(messages, "usages", "tempmuteip");
        source.sendMessage(text);
        return Command.SINGLE_SUCCESS;
    }

    @SuppressWarnings("SameReturnValue")
    private int tempmuteip(CommandContext<CommandSource> context) {
        final CommandSource source = context.getSource();
